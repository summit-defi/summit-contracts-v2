...
describe("OASIS Pools", function() {
  ...
  describe('- Summit Enabled', async function () {
    ...
    it('ALLOCPOINT: No rewards are earned on 0 alloc point', async function () {
      ...
      const userHarvestable0 = await subCartGet.poolClaimableRewards(summitToken.address, OASIS, user1.address)
      await mineBlock()
      const userHarvestable1 = await subCartGet.poolClaimableRewards(summitToken.address, OASIS, user1.address)
